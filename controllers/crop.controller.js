...
module.exports.payment = async (req, res) => {
  ...
  try {
    ...
    // Fetch unpaid crops (oldest first)
    const crops = await Crop.find({
      _id: { $in: cropIds },
      pendingAmount: { $gt: 0 },
      dealerId: req.user._id,
    ...
    for (const crop of crops) {
      ...
      /* ---- Store PER-CROP Inventory Snapshot ---- */
      paymentDetails.push({
        cropId: crop._id,
    ...
    req.flash("success", "Multi-crop payment saved successfully");
    res.redirect(
      personType === "FARMER"
        ? `/farmer/${personBusinessId}/crops`
        : `/buyer/${personBusinessId}/crops`
      ...
      [
        {
          ...
          payments: reversalPayments, // ✅ reversal clears its own balance
        },
      ],
      { session }
    );
